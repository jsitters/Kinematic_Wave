...
alpha_data=pd.DataFrame(data=np.zeros((len(time2d)-1, length.count())), columns=list(model_segs.name))
...
#        model_segs['Perimeter'][j]=w[j]+1*depth_data[j]*(1+z**2)**0.5    
...
plt.plot(time2d,Qout[:,0], 'b', label='Boundary Flow')
plt.plot(time2d,Qout[:,1], 'c', label='First')
plt.plot(time2d,Qout[:,2], 'g', label='Second')
plt.plot(time2d,Qout[:,3], 'r', label='Third')
plt.plot(time2d,Qout[:,4], 'k', label='Fourth')
plt.ylabel('Flow')
